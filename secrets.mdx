...
Secrets are encrypted environment variables associated with your BuildBuddy organization. Secrets can be used in actions executed with[remote execution](/docs/remote-build-execution) as well as [BuildBuddy Workflows](/docs/workflows-introduction).

BuildBuddy encrypts secrets with a[libsodium sealed box](https://libsodium.gitbook.io/doc/public-key%5Fcryptography/sealed%5Fboxes). Secrets are encrypted before they are sent to BuildBuddy's servers, and stay encrypted until they are used.

## Why use secrets?[](#why-use-secrets "Direct link to Why use secrets?")

Builds that are executed remotely on BuildBuddy's servers may occasionally need access to an API key or other credentials. For example, you may want to [pass credentials for a Docker image](/docs/rbe-platforms#passing-credentials-for-docker-images).
...
## Defining secrets[](#defining-secrets "Direct link to Defining secrets")

Secrets can be added to your organization using the Secrets page in[settings](https://app.buildbuddy.io/settings/org/secrets).
...
## Getting secret values[](#getting-secret-values "Direct link to Getting secret values")

### Bazel actions[](#bazel-actions "Direct link to Bazel actions")

To opt a specific action into secrets, you can define the remote exec property `include-secrets=true`. We recommend doing this per-action to avoid exposing secrets to actions that do not need them.
...
[BuildBuddy workflows](/docs/workflows-introduction) do not need additional configuration to use secrets; they receive secrets by default as long as the workflow is being triggered on behalf of a trusted collaborator in the repository.

Workflow secrets are accessed via environment variables, in the same way as normal Bazel actions shown above.

## Short-lived secrets[](#short-lived-secrets "Direct link to Short-lived secrets")
...
Secrets are encrypted on the client-side using[libsodium](https://doc.libsodium.org/), which is based on[NaCl](http://nacl.cr.yp.to/).
