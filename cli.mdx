...
## Installing for a project[](#installing-for-a-project "Direct link to Installing for a project")

If you're already using Bazelisk, you can easily install the BuildBuddy CLI for your entire project by running:

```
echo "$(echo "buildbuddy-io/5.0.21"; cat .bazelversion)" > .bazelversion

```

This will simply prepend `buildbuddy-io/5.0.21` on a new line above your `.bazelversion` file like so:

```
buildbuddy-io/5.0.21
5.3.2

```

The version 5.0.21 of the BuildBuddy CLI will now automatically be used when you type `bazel` or `bazelisk` and continue to use the Bazel version specified on the second line of your `.bazelrc` file.
