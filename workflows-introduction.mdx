...
None of these pipeline-based approaches are well suited for Bazel's approach to dependency management and remote build execution, which involves generating a [Directed Acyclic Graph](https://en.wikipedia.org/wiki/Directed%5Facyclic%5Fgraph) of all build and test actions. Bazel's approach allows for optimal parallelization and caching of these actions. It also enables rebuilding and retesting only affected targets, saving both engineering time and compute resources.

## Introducing a Bazel-focused CI Solution[](#introducing-a-bazel-focused-ci-solution "Direct link to Introducing a Bazel-focused CI Solution")
...
But because BuildBuddy Workflows were built for Bazel repos and tightly integrated with BuildBuddy RBE and Remote Caching, they are **_really fast_**.

## How fast are BuildBuddy Workflows?[](#how-fast-are-buildbuddy-workflows "Direct link to How fast are BuildBuddy Workflows?")
...
![overlapping histogram comparing BuildBuddy and GitHub actions](/images/workflows-5e3a4008b17abce93baa478089e90f9b.png)

## How did we make BuildBuddy Workflows so fast?[](#how-did-we-make-buildbuddy-workflows-so-fast "Direct link to How did we make BuildBuddy Workflows so fast?")
...
### Fast connection to BuildBuddy RBE[](#fast-connection-to-buildbuddy-rbe "Direct link to Fast connection to BuildBuddy RBE")
...
With GitHub actions or other CI solutions, the network connection might be fast (particularly after the recent network optimizations we made in[BuildBuddy v2](https://blog.buildbuddy.io/blog/introducing-buildbuddy-v2)) — but not nearly as fast as having workflow runners on the same local network as BuildBuddy itself.

### Hosted, Warm, Bazel instances[](#hosted-warm-bazel-instances "Direct link to Hosted, Warm, Bazel instances")

Once you have a sufficiently fast RBE and Remote Caching setup, and have removed network bottlenecks — the CI bottleneck often becomes Bazel's [analysis phase](https://docs.bazel.build/versions/main/glossary.html#analysis-phase).
...
## How do I use BuildBuddy Workflows?[](#how-do-i-use-buildbuddy-workflows "Direct link to How do I use BuildBuddy Workflows?")

BuildBuddy Workflows are launching today, in Beta, for all GitHub users. You can get started with BuildBuddy Workflows by checking out our [setup guide](https://docs.buildbuddy.io/docs/workflows-setup/). If you've already linked your GitHub account to BuildBuddy, it'll only take about 30 seconds to enable Workflows for your repo — just select a repo to link, and we'll take care of the rest!