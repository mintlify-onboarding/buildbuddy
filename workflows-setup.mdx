...
## Enable workflows for a repo[](#enable-workflows-for-a-repo "Direct link to Enable workflows for a repo")
...
## Configuring your workflow[](#configuring-your-workflow "Direct link to Configuring your workflow")

To learn how to change the default configuration, see [workflows configuration](/docs/workflows-config).

## Setting up branch protection rules[](#setting-up-branch-protection-rules "Direct link to Setting up branch protection rules")
...
## Building in the workflow runner environment[](#building-in-the-workflow-runner-environment "Direct link to Building in the workflow runner environment")
...
The Dockerfile we use to build the image (at `HEAD`) is [here](https://github.com/buildbuddy-io/buildbuddy/blob/master/enterprise/dockerfiles/ci%5Frunner%5Fimage/Dockerfile).

If you plan to use the Ubuntu 20.04 image (requires [advanced configuration](/docs/workflows-config#linux-image-configuration)), use`"gcr.io/flame-public/rbe-ubuntu20-04-workflows:latest"` in the command above.