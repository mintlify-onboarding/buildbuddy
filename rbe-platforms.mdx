...
## BuildBuddy default[](#buildbuddy-default "Direct link to BuildBuddy default")
...
## Using a custom Docker image[](#using-a-custom-docker-image "Direct link to Using a custom Docker image")
...
### ENTRYPOINT and CMD[](#entrypoint-and-cmd "Direct link to ENTRYPOINT and CMD")
...
### Passing credentials for Docker images[](#passing-credentials-for-docker-images "Direct link to Passing credentials for Docker images")
...
Some cloud providers may also allow the use of long-lived tokens, which can also be used in remote headers. For example, GCR allows setting a username of `_json_key` and then using a service account's[JSON-format private key](https://cloud.google.com/iam/docs/creating-managing-service-account-keys)as the password. Note that remote headers cannot have newlines; the command `tr '\n' ' '` is used in this example to remove them:
...
## Specifying a custom executor pool[](#specifying-a-custom-executor-pool "Direct link to Specifying a custom executor pool")
...
For instructions on how to deploy custom executor pools, we the [RBE Executor Pools docs](/docs/rbe-pools).

## Target level execution properties[](#target-level-execution-properties "Direct link to Target level execution properties")
