...
## The Build Event Protocol upload timed out[](#the-build-event-protocol-upload-timed-out "Direct link to The Build Event Protocol upload timed out")

This error means the `bes_timeout` [flag](https://docs.bazel.build/versions/master/command-line-reference.html#flag--bes%5Ftimeout) is likely set to a value that's not long enough for bazel to finish uploading all build artifacts.
...
## Waiting for build events upload[](#waiting-for-build-events-upload "Direct link to Waiting for build events upload")
