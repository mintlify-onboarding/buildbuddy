...
## Deploying a BuildBuddy cluster[](#deploying-a-buildbuddy-cluster "Direct link to Deploying a BuildBuddy cluster")
...
You can follow the standard [Enterprise RBE Setup](/docs/enterprise-rbe) instructions to get your cluster up and running.

## Mac environment setup[](#mac-environment-setup "Direct link to Mac environment setup")

Once you have a BuildBuddy cluster deployed with RBE enabled, you can start setting up your Mac executors.

### Downloading Xcode[](#downloading-xcode "Direct link to Downloading Xcode")
...
### Installing Xcode[](#installing-xcode "Direct link to Installing Xcode")
...
### Installing Homebrew[](#installing-homebrew "Direct link to Installing Homebrew")
...
## Installing the BuildBuddy Mac executor[](#installing-the-buildbuddy-mac-executor "Direct link to Installing the BuildBuddy Mac executor")

Now that the environment is configured, we can download and install the BuildBuddy Mac executor.

### Download the BuildBuddy executor[](#download-the-buildbuddy-executor "Direct link to Download the BuildBuddy executor")
...
### Make the executor executable[](#make-the-executor-executable "Direct link to Make the executor executable")
...
### Create directories[](#create-directories "Direct link to Create directories")
...
### Create config file[](#create-config-file "Direct link to Create config file")
...
Make sure to replace _YOUR\_USERNAME_ with your Mac username and _YOUR\_BUILDBUDDY\_CLUSTER\_URL_ with the grpc url the BuildBuddy cluster you deployed. If you deployed the cluster without an NGINX Ingress, you'll need to update the protocol to grpc:// and the port to 1985.

### Create a Launch Agent .plist file[](#create-a-launch-agent-plist-file "Direct link to Create a Launch Agent .plist file")
...
Make sure to replace _YOUR\_USERNAME_ with your Mac username and _YOUR\_MACS\_NETWORK\_ADDRESS_ with the IP address or DNS name of the Mac.
...
### Update Launch Agent plist permissions[](#update-launch-agent-plist-permissions "Direct link to Update Launch Agent plist permissions")
...
### Start the Launch Agent[](#start-the-launch-agent "Direct link to Start the Launch Agent")
...
## Optional setup[](#optional-setup "Direct link to Optional setup")

### Optional: Enable Autologin[](#optional-enable-autologin "Direct link to Optional: Enable Autologin")
...
### Optional: Install Java[](#optional-install-java "Direct link to Optional: Install Java")
...
### Optional: Increase the maximum number of open files[](#optional-increase-the-maximum-number-of-open-files "Direct link to Optional: Increase the maximum number of open files")
...
### Optional: Log rotation[](#optional-log-rotation "Direct link to Optional: Log rotation")
