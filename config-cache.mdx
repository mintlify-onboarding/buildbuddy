...
* `in_memory:` Whether or not to use the in\_memory cache.
...
### GCS & Redis (Enterprise only)[](#gcs--redis-enterprise-only "Direct link to GCS & Redis (Enterprise only)")
...
### S3 (Enterprise only)[](#s3-enterprise-only "Direct link to S3 (Enterprise only)")
...
### Minio (Enterprise only)[](#minio-enterprise-only "Direct link to Minio (Enterprise only)")
