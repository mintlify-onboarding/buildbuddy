...
## Configuring your install[](#configuring-your-install "Direct link to Configuring your install")
...
GCP's [n2-standard-8 machines](https://cloud.google.com/compute/docs/machine-types#n2%5Fstandard%5Fmachine%5Ftypes) or similar are a good place to start. For information on scaling up and down your deployments, see the **Configuring resource** section.
...
You should now see a remote build execution checkbox and can try your first remote build execution. Make sure your RBE [toolchains and platforms](/docs/rbe-setup) are correctly configured.

## Configuring resources[](#configuring-resources "Direct link to Configuring resources")
...
Note: if you scale beyond one app replica, make sure you're using MySQL instead of SQLite, and GCS/S3 instead of disk storage. For information on how to configure these, see the [enterprise configuration guide](/docs/enterprise-config). Scaling the number of executor replicas does not have these requirements.
...
For more configuration options beyond RBE, like authentication and storage options, see our [configuration docs](/docs/config) and our [enterprise configuration guide](/docs/enterprise-config).
