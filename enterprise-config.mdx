...
2. Create a new user with full access to that database
3. Put the username, password, IP address of your MySQL server, and database name into the BuildBuddy data\_source connection string:
...
### Default Redis Target[](#default-redis-target "Direct link to Default Redis Target")
...
### GCS Based Cache / Object Storage / Redis[](#gcs-based-cache--object-storage--redis "Direct link to GCS Based Cache / Object Storage / Redis")
...
### Authentication Provider Integration[](#authentication-provider-integration "Direct link to Authentication Provider Integration")
...
Here’s another example of Google login using credentials obtained from: [https://console.developers.google.com/apis/credentials](https://console.developers.google.com/apis/credentials)
...
### Certificate Based Authentication[](#certificate-based-authentication "Direct link to Certificate Based Authentication")
...
You'll also need to deploy executors to handle remote builds. The recommended way of deploying these is using our [Enterprise Helm Chart](/docs/enterprise-helm).

For more information on configuring on-prem RBE, see our [enterprise on-prem RBE setup docs](/docs/enterprise-rbe).
...
For more information on configuring BuildBuddy, see our [Configuration docs](/docs/config). If you have questions please don’t hesitate to email us at [setup@buildbuddy.io](mailto:setup@buildbuddy.io) or ping us on our [Slack channel](https://community.buildbuddy.io).