...
description: "Additional metadata can be sent up with your Bazel invocation to give BuildBuddy more information about your build."
---
...
### Build metadata[](#build-metadata "Direct link to Build metadata")

The first method is simple - just provide the repo URL with Bazel's build\_metadata flag with the key `REPO_URL`. You can do this by adding the following line to your `.bazelrc`:
...
### Workspace info[](#workspace-info "Direct link to Workspace info")
...
## Git Branch[](#git-branch "Direct link to Git Branch")
...
### Build metadata[](#build-metadata-1 "Direct link to Build metadata")

You can provide the current git branch with Bazel's build\_metadata flag with the key `BRANCH_NAME`. You can do this by adding the flag to your bazel invocations:
...
### Workspace info[](#workspace-info-1 "Direct link to Workspace info")
...
### Build metadata[](#build-metadata-2 "Direct link to Build metadata")

You can provide the current commit SHA with Bazel's build\_metadata flag with the key `COMMIT_SHA`. You can do this by adding the flag to your bazel invocations:
...
### Workspace info[](#workspace-info-2 "Direct link to Workspace info")
...
## Test groups[](#test-groups "Direct link to Test groups")
...
### Build metadata[](#build-metadata-3 "Direct link to Build metadata")

You can provide your user with Bazel's build\_metadata flag with the key `USER`. You can do this by adding the flag to your bazel invocations:
...
### Workspace info[](#workspace-info-3 "Direct link to Workspace info")
...
### Build metadata[](#build-metadata-4 "Direct link to Build metadata")

You can provide your user with Bazel's build\_metadata flag with the key `HOST`. You can do this by adding the flag to your bazel invocations:
...
### Workspace info[](#workspace-info-4 "Direct link to Workspace info")
...
### Build metadata[](#build-metadata-5 "Direct link to Build metadata")

You can provide a custom pattern with Bazel's build\_metadata flag with the key `PATTERN`. You can do this by adding the flag to your bazel invocations:
...
### Workspace info[](#workspace-info-5 "Direct link to Workspace info")
...
## Environment variable redacting[](#environment-variable-redacting "Direct link to Environment variable redacting")

By default, all environment variables are redacted by BuildBuddy except for `USER`, `GITHUB_ACTOR` `GITHUB_REPOSITORY`, `GITHUB_SHA`, `GITHUB_RUN_ID`, `BUILDKITE_BUILD_URL`, `BUILDKITE_JOB_ID`, `CIRCLE_REPOSITORY_URL`, `GITHUB_REPOSITORY`, `BUILDKITE_REPO`, `TRAVIS_REPO_SLUG`, `GIT_URL`,`CI_REPOSITORY_URL`, `REPO_URL`, `CIRCLE_SHA1`, `GITHUB_SHA`, `BUILDKITE_COMMIT`, `TRAVIS_COMMIT`, `GIT_COMMIT`, `CI_COMMIT_SHA`, `COMMIT_SHA`, `CI`, `CI_RUNNER`, `CIRCLE_BRANCH`, `GITHUB_HEAD_REF`, `BUILDKITE_BRANCH`, `TRAVIS_BRANCH`, `GIT_BRANCH`, `CI_COMMIT_BRANCH`, `GITHUB_REF`, which are displayed in the BuildBuddy UI.
